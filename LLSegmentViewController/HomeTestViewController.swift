...
class HomeTestViewController: LLSegmentViewController {
...
extension HomeTestViewController{
    ...
    func layoutContentView() {
        let segmentCtlFrame = CGRect.init(x: 0, y: 0, width: UIScreen.main.bounds.width, height: 44)
        let containerFrame = CGRect.init(x: 0, y: 44, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height - segmentCtlFrame.height - mSafeBottomMargin() - 49 - mTopHeight())
        self.layoutInfo.segmentControlPositionType = .customFrame(segmentCtlFrame: segmentCtlFrame, containerFrame: containerFrame)
